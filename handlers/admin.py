...
async def commands_up(message : types.Message):
...
async def load_photo(message : types.Message, state=FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply('Name current photo')
...
async def load_name(message : types.Message, state=FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply('Description of current photo')
...
async def load_descr(message : types.Message, state=FSMContext):
    if message.from_user.id == ID:
        ...
        await FSMAdmin.next()
        await message.reply('Year when it was')
...
async def del_callback_run(callback_querry: types.CallbackQuery):
    ...
    await callback_querry.answer(text=f'{callback_querry.data.replace("del ", "")} deleted.', show_alert=True)
...
async def delete_item(message: types.Message):
    if message.from_user.id == ID:
        ...
        for ret in read:
            await bot.send_photo(message.from_user.id, ret[0], f'{ret[1]}\Description: {ret[2]}\nYear {ret[-1]}')
...
def register_handlers_admin(dp : Dispatcher):
    dp.register_message_handler(commands_up, commands='Upload', state=None)  
    dp.register_message_handler(load_photo, content_types=['photo'], state=FSMAdmin.photo)
    dp.register_message_handler(load_name, state=FSMAdmin.name)
    dp.register_message_handler(load_descr, state=FSMAdmin.description)
